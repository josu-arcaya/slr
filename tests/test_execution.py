...
class TestSqlAlchemyORM(unittest.TestCase):
    ...
    def test_init(self):
        self.assertIsNotNone(self.orm.db._engine, "Error in __init__: 'Engine' has not been correctly initialized")
        self.assertIsNotNone(self.orm.db._Session, "Error in __init__: 'Session' has not been correctly initialized")

    def test_get_session(self):
        session = self.orm.db.get_session()
        self.assertIsNotNone(session, "Error in get_session: Session was not obtained correctly")
    ...
    def test_set_get_publisher_by_eissn(self):
        ...
        self.assertIsNotNone(result, "Error in set_publisher_by_eissn / get_publisher_by_eissn")
    ...
    def test_save_and_get_doi(self):
        documents = [("Title1", "Abstract1", "Keyword1", "Author1", "2022-01-01", "DOI1", "EID1", "Publication1",
                      "ISSN1", "EISSN1", "Type1", "SubType1", "Query1", "Source1", "Country1", 10)]
        self.orm.save(documents)

        result = self.orm.get_doi()
        self.assertNotEqual(result, [], "Error in save / get_doi")

    # Author publishing test and ISSN consult test without assigned publisher in IssnPublisher
    def test_set_publisher_get_empty_publisher(self):
        try:
            documents = [("Title2", "Abstract2", "Keyword2", "Author2", "2022-01-02", "DOI2", "EID2", "Publication2",
                          "ISSN2", "EISSN2", "Type2", "SubType2", "Query2", "Source2", "Country2", 10)]
            self.orm.save(documents)
            publishers = [('ISSN2', None)]
            result_empty_publisher = self.orm.get_empty_publisher()
            self.assertEqual(result_empty_publisher, [], "Error in set_publisher / get_empty_publisher")
        except AssertionError:
            LOGGER.error(f"Error in set_empty_publisher. The editor can't be null: (publisher = Column(nullable=False))\n")

    # Test of ISSN query of publishers without an assigned value in IssnPublisher
    def test_get_all_issn_without_publisher(self):
        documents = [("Title3", "Abstract3", "Keyword3", "Author3", "2022-01-03", "DOI3", "EID3", "Publication3",
                      "ISSN3", "EISSN3", "Type3", "SubType3", "Query3", "Source3", "Country3", 10)]
        self.orm.save(documents)

        result_issn_without_publisher = self.orm.get_all_issn_without_publisher()
        expected_result = [("EID3", "ISSN3", "EISSN3")]
        self.assertEqual(result_issn_without_publisher, expected_result, "Error in get_all_issn_without_publisher")

    # Query test of countries without an assigned continent
    def test_get_empty_continents(self):
        documents = [("Title4", "Abstract4", "Keyword4", "Author4", "2022-01-04", "DOI4", "EID4", "Publication4",
    ...
    def test_set_doi_eurl(self):
        ...
        try:
            ...
            self.fail("Error in set_doi_eurl")
    ...
    def test_set_get_openaccess(self):
        ...
        try:
            ...
            if result is not None:
                self.assertEqual(result, ['EID5'], "Error in get_empty_openaccess")

            self.orm.set_openaccess('EID5', 'openaccess')
        except (NameError, CompileError, SystemExit):
            LOGGER.error(f" Error in test_set_get openaccess. The name 'StudySelection' is not defined.\n")
